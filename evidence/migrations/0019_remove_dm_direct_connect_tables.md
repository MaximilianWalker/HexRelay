...
- Latest meaningful change: 2026-05-08 added validation evidence for dropping obsolete DM pairing nonce and endpoint-card tables after the node/server-routed E2EE envelope pivot.
...
- Expected outcome: DB startup can include migration `0019_remove_dm_direct_connect_tables` after earlier direct-DM migrations, while runtime API and contract surfaces no longer reference pairing nonce or endpoint-card tables.
- Actual outcome: pass.
...
- Rollback or restore command(s): N/A for intentional destructive cleanup of retired direct-DM tables; recovery uses database backup/restore before migration application if obsolete data must be inspected.
...
- Decision: pass
- Notes: This migration deliberately removes persisted direct-DM bootstrap/control leftovers after runtime and contract surfaces moved to node/server-routed E2EE envelope delivery.
