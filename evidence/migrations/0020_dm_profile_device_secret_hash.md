...
- Command(s) executed: `cargo check -p api-rs`, `cargo check -p realtime-rs`, `cargo test -p api-rs fanout_ack`, `cargo test -p api-rs dm_fanout`, `cargo test -p api-rs dm_envelope_dispatch_ack_persists_through_realtime_websocket`, `cargo test -p realtime-rs dm`, `npm --prefix apps/web test -- lib/api.test.ts lib/dm-realtime.test.ts`, `node scripts/validators/contract-parity.mjs HEAD HEAD`, `node scripts/validators/dm-transport-policy.mjs`.
