...
- Command(s) executed: `cargo fmt --all`, `cargo clippy -p api-rs --all-targets -- -D warnings`, `cargo test -p api-rs server_channel_messages -- --nocapture`, `cargo test -p api-rs -- --nocapture`, `bash scripts/validators/contract-parity.sh`, `bash scripts/validators/docs-index-freshness.sh`
