use api_rs::{
    ...
    domain::server_identity::{
        generate_server_identity, server_identity_generate_usage, ServerIdentityGenerateCliOptions,
...
fn main() {
    if let Err(error) = run() {
        eprintln!("[generate-server-identity] ERROR: {error}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), String> {
    ...
    if raw_args.iter().any(|arg| arg == "--help" || arg == "-h") {
        println!("{}", server_identity_generate_usage());
        return Ok(());
    }

    let mut options =
        ServerIdentityGenerateCliOptions::parse(raw_args).map_err(|error| error.to_string())?;
    ...
    if options.generate_options.ttl_seconds > options.generate_options.max_ttl_seconds {
        return Err(format!(
            "--ttl-seconds must be less than or equal to the server descriptor TTL ceiling ({})",
            options.generate_options.max_ttl_seconds
        ));
    }

    let issued_at_epoch_seconds = current_epoch_seconds()?;
    let (output, _) = generate_server_identity(&options.generate_options, issued_at_epoch_seconds)
        .map_err(|error| error.to_string())?;

    if options.json_compact {
        println!(
            "{}",
            serde_json::to_string(&output).map_err(|error| format!(
                "failed to serialize generated server identity: {error}"
            ))?
        );
    } else {
        println!(
