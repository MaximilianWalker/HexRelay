...
pub async fn insert_invite(
    ...
    sqlx::query(
        "
        INSERT INTO invites (invite_id, token, mode, creator_identity_id, server_id, expires_at, max_uses, uses)
...
pub async fn load_invite_for_update(
    ...
    let row = sqlx::query(
        "
        SELECT invite_id, creator_identity_id, server_id, expires_at, max_uses, uses
        FROM invites
        WHERE token = $1
        FOR UPDATE
        ",
