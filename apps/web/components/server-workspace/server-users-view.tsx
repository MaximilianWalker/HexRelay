...
import type { ServerRoleGroup } from "./server-workspace-types";

import styles from "@/app/surfaces.module.css";

type ServerUsersViewProps = {
  authorHandle: (identityId: string) => string;
  authorLabel: (identityId: string) => string;
  formatTimestamp: (value: string) => string;
  hasSession: boolean;
  identityId: string;
  roleGroups: ServerRoleGroup[];
};

export function ServerUsersView({
  authorHandle,
  authorLabel,
  formatTimestamp,
  hasSession,
