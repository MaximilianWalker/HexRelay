...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --json)
      ...
      ;;
    --help|-h)
      echo "Usage: stop.sh [--runtime-profile single|dual|triple] [--json]"
      exit 0
      ;;
    *)
...
if [[ -n "$RUNTIME_PROFILE" && "$ACTIVE_PROFILE" != "$RUNTIME_PROFILE" ]]; then
  echo "[stop] ERROR: Active runtime profile is '$ACTIVE_PROFILE', not '$RUNTIME_PROFILE'." >&2
  exit 1
fi

STOPPED_JSON="[]"
while IFS=$'\t' read -r instance_id service pid; do
  stopped=false
...
done < <(node -e 'const fs=require("node:fs"); const s=JSON.parse(fs.readFileSync(process.argv[1], "utf8")); for (const i of s.instances ?? []) for (const [service, pid] of [["api", i.apiPid], ["realtime", i.realtimePid], ["web", i.webPid]]) console.log([i.id, service, pid].join("\t"));' "$STATE_PATH")
