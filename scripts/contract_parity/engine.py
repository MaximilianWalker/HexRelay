...
def extract_api_runtime_inventory(path: str) -> str:
    ...
    sys.argv = ["extract_api_runtime_inventory", path]
    buffer = io.StringIO()
    try:
...
def extract_openapi_contract_inventory(path: str) -> str:
    original_argv = sys.argv[:]
    sys.argv = ["extract_openapi_contract_inventory", path]
    buffer = io.StringIO()
    try:
        with contextlib.redirect_stdout(buffer):
...
def extract_api_runtime_error_codes(*args: str) -> str:
    original_argv = sys.argv[:]
    sys.argv = ["extract_api_runtime_error_codes", *args]
    buffer = io.StringIO()
    try:
        with contextlib.redirect_stdout(buffer):
...
def extract_openapi_contract_error_codes(path: str) -> str:
    original_argv = sys.argv[:]
    sys.argv = ["extract_openapi_contract_error_codes", path]
    buffer = io.StringIO()
    try:
        with contextlib.redirect_stdout(buffer):
...
def validate_api_semantic_contracts(contract_path: str) -> int:
    original_argv = sys.argv[:]
    sys.argv = ["validate_api_semantic_contracts", contract_path]
    buffer = io.StringIO()
    exit_code = 0
    try:
        with contextlib.redirect_stdout(buffer):
...
def extract_realtime_runtime_events(path: str) -> str:
    original_argv = sys.argv[:]
    sys.argv = ["extract_realtime_runtime_events", path]
    buffer = io.StringIO()
    try:
        with contextlib.redirect_stdout(buffer):
...
def extract_asyncapi_contract_events(path: str) -> str:
    original_argv = sys.argv[:]
    sys.argv = ["extract_asyncapi_contract_events", path]
    buffer = io.StringIO()
    try:
        with contextlib.redirect_stdout(buffer):
...
def extract_realtime_runtime_error_codes(*args: str) -> str:
    original_argv = sys.argv[:]
    sys.argv = ["extract_realtime_runtime_error_codes", *args]
    buffer = io.StringIO()
    try:
        with contextlib.redirect_stdout(buffer):
...
def extract_asyncapi_contract_error_codes(path: str) -> str:
    original_argv = sys.argv[:]
    sys.argv = ["extract_asyncapi_contract_error_codes", path]
    buffer = io.StringIO()
    try:
        with contextlib.redirect_stdout(buffer):
