...
try:
    from .rules import (
        ...
        API_ERROR_REQUIRED_FIELDS,
        API_ERROR_SCHEMA_NAME,
        API_HANDLER_DIR,
        API_MODELS_PATH,
        API_ROUTER_PATH,
        AUTH_PARAM_MARKERS,
        AUTH_SESSION_SECURITY_SCHEMES,
        AUTHORIZER_ERROR_STATUSES,
        CSRF_HEADER_NAME,
        CSRF_HEADER_SCHEMA_TYPE,
        INTERNAL_TOKEN_REQUIRED_HEADERS,
        JSON_REQUEST_MEDIA_TYPE,
        ...
        TRACKED_ERROR_EXAMPLE_STATUS_PATTERN,
        TRACKED_ERROR_SCHEMA_STATUSES,
        TRACKED_ERROR_STATUS_PATTERN,
        ...
        UNAUTHORIZED_HELPER_NAME_PATTERN,
    )
    from .rust_parsing import (
        extract_path_param_details,
        extract_query_type,
        extract_request_body_schema,
        extract_response_body_schema,
        has_json_response_builder,
        has_request_body_extractor,
    )
    ...
    from rust_parsing import (  # type: ignore
        extract_path_param_details,
        extract_query_type,
        extract_request_body_schema,
        extract_response_body_schema,
...
def validate_api_semantic_contracts(contract_path_str: str) -> int:
        ...
        for path in paths:
            ...
            for match in pattern.finditer(text):
                ...
                functions[function_id] = {
                    ...
                    'request_body_schema': extract_request_body_schema(params, REQUEST_SCHEMA_ALIASES),
                    'response_body_schema': extract_response_body_schema(return_type, body, RESPONSE_SCHEMA_ALIASES),
    ...
    def infer_success_status(handler_id, functions, local_lookup, stack=None):
        ...
        if 'StatusCode::ACCEPTED' in body:
            return '202'
        if has_json_response_builder(body, RESPONSE_SCHEMA_ALIASES):
            return '200'
    ...
    def infer_success_body_kind(handler_id, functions, local_lookup, stack=None):
        ...
        if 'Json<' in return_type:
            return 'json'
        if has_json_response_builder(function.get('body', ''), RESPONSE_SCHEMA_ALIASES):
            return 'json'
    ...
    router_text = API_ROUTER_PATH.read_text()
    handler_paths = sorted(API_HANDLER_DIR.glob('*.rs'))
    models_path = API_MODELS_PATH
