...
- Latest meaningful change: 2026-05-20 recorded the destructive singleton server storage migration and updated the server authority regression risk.
...
| D-007 | E2EE DM envelope delivery conformance profile | Internal | ready | DM baseline could drift into server-readable payloads, excess metadata, private-key custody, or retired server-bypassing client DM surfaces without repeatable evidence | Core/API/QA | Conformance must prove ciphertext-only server-to-server/message-server handling, client-only plaintext/private keys, deterministic delivery-metadata retention, metadata-only abuse controls, and absence of server-bypassing client DM transport/bootstrap surfaces |
...
|---|---|---|---|---|---|
| R-001 | Scope creep in decentralization scope | high | medium | Keep MVP to federation-lite signed registry discovery plus explicit server-to-server policy boundaries; defer Kademlia/HyParView/Plumtree until public opt-in networks need them | Product |
...
| R-007 | User identity or private-server scraping via discovery/friend workflows | high | medium | Enforce mediated friend requests, signed opt-in server descriptors, descriptor-scope validation, rate limits, denylists, and bootstrap release only on accepted requests | API |
| R-008 | DM encrypted-envelope delivery leaks plaintext, private keys, or excess metadata through servers/message servers | high | medium | Enforce ciphertext-only schemas, client-only decryption/key storage, minimal delivery metadata, 30-day fanout metadata retention, 7-day outbound forwarding metadata retention, metadata-only rate limits, and CI guardrails that reject plaintext mailbox/relay semantics | Core/API/Security |
...
- Last reviewed: 2026-05-20 (updated after destructive singleton server storage migration).
...
| DEC-004 | Post-MVP discovery roadmap is hybrid: private/trusted discovery first, federation registries supported, user-consented server introductions allowed by descriptor policy, and decentralized server discovery optional | accepted | `docs/product/01-mvp-plan.md` |
...
| DEC-007 | DM delivery baseline uses servers/message servers in the server-to-server network for E2EE envelope store-and-forward; DM plaintext and private keys remain client/device-only | accepted | `docs/product/01-mvp-plan.md` |
...
| DEC-011 | Server-to-server P2P topology is a dynamic policy graph with no primary-server assumption; discovery, peering, relay, delivery, and storage permissions are separate | accepted | `docs/architecture/04-communication-networking-layer-plan.md` |
| DEC-012 | DM delivery metadata retention is separate from canonical encrypted DM history; abuse controls are sender/device/server scoped and do not inspect plaintext | accepted | `docs/product/01-mvp-plan.md` |
| DEC-013 | One user-facing server maps to one separately runnable server runtime authority; one API database stores only that server's singleton local-server state | accepted | `docs/architecture/adr-0004-server-authority.md` |
