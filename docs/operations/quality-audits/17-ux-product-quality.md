...
|---|---|---|---|---|---|---|
| QA-17-20260514-dm-workspace-send-not-wired | P1 | confirmed | DM workspace exposes a composer and send button, but accepted contacts still cannot send or load E2EE DM history. | `docs/product/02-prd.md:109-118` requires E2EE private messaging with encrypted outbound payloads, delivery metadata, and device convergence; `docs/product/02-prd.md:188-195` requires server-node envelope delivery and delivery diagnostics; `docs/product/08-screen-state-spec.md:43` requires DM workspace states, and `docs/product/08-screen-state-spec.md:88-90` requires deterministic policy/outbox states. In `apps/web/app/contacts/[contactId]/messages/page.tsx:160-163`, Send reports "delivery is not wired in this build yet"; `apps/web/app/contacts/[contactId]/messages/page.tsx:221-223` says a future backend slice will load E2EE history and send encrypted envelopes. | Propose and approve the DM workspace flow before implementation, then wire thread history, encrypted send, and the documented blocked/policy/retry/reconnect states without introducing out-of-scope preflight UI. | 2026-05-14T12:49:36Z |
