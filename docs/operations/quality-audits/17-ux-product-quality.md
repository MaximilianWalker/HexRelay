...
|---|---|---|---|---|---|---|
| QA-17-20260514-dm-workspace-send-not-wired | P1 | approval_pending | DM workspace exposes a composer and send button, but accepted contacts still cannot send or load E2EE DM history. | `docs/product/02-prd.md:109-118` requires E2EE private messaging with encrypted outbound payloads, delivery metadata, and device convergence; `docs/product/02-prd.md:188-195` requires server-node envelope delivery and delivery diagnostics; `docs/product/08-screen-state-spec.md:43` requires DM workspace states, and `docs/product/08-screen-state-spec.md:88-90` requires deterministic policy/outbox states. In `apps/web/app/contacts/[contactId]/messages/page.tsx:160-163`, Send reports "delivery is not wired in this build yet"; `apps/web/app/contacts/[contactId]/messages/page.tsx:221-223` says a future backend slice will load E2EE history and send encrypted envelopes. `docs/planning/dm-workspace-delivery-implementation-plan.md` now defines the approval-pending flow package, merged in PR #165. | Wait for explicit `DMW-APP-*` approval before implementation; keep this finding out of selector-active status until approval exists or the proposed flow/copy/control package changes. | 2026-05-15T05:04:39Z |
...
|---|---|---|
| 2026-05-15T05:04:39Z | Codex automation | Reclassified `QA-17-20260514-dm-workspace-send-not-wired` as `approval_pending` after merged plan-only PR #165 so autonomous development does not repeatedly select unapproved DM workspace UX implementation. |
