...
- last_audited: 2026-05-15T05:04:39Z
...
|---|---|---|---|---|---|---|
| QA-17-20260514-servers-hub-state-actions-incomplete | P2 | watch | Servers Hub lacks user-actionable empty/error/search states and required card actions. | `docs/product/07-ui-navigation-spec.md:73-76` requires a searchable server card grid with favorites/unread/muted filters plus card actions for open server, open settings, and pin/unpin; `docs/product/07-ui-navigation-spec.md:90-93` requires hub searches to be focusable, empty states to include a next action, deep links, and persisted hub preferences; `docs/product/08-screen-state-spec.md:44` requires loading, empty, search_no_results, permission_denied, and error states. `apps/web/app/servers/page.tsx:162-178` renders cards with only a server link and metadata, then prints `state: {state}` for every non-card condition instead of actionable state UI. `docs/planning/navigation-implementation-plan.md:101-120` defines the approval-pending Servers Hub flow/controls, and `docs/planning/navigation-implementation-plan.md:297-312` keeps `NAV-APP-01` pending as the approval gate for Servers Hub filters, card actions, and state copy. | Do not implement runtime Servers Hub UX until an explicit approval reference with non-pending `NAV-APP-01` exists; after approval, implement the scoped `NAV-02` Servers Hub slice and cite the approval evidence. | 2026-05-18T10:01:52Z |
...
|---|---|---|
| 2026-05-18T10:01:52Z | Codex automation | Reclassified `QA-17-20260514-servers-hub-state-actions-incomplete` as `watch`: the runtime gap is still present, but `docs/planning/navigation-implementation-plan.md` already contains the approval-pending Servers Hub package and `NAV-APP-01` remains pending, so UI implementation is blocked by the explicit UX approval gate. |
