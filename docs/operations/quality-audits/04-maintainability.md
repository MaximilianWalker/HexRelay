...
| QA-04-20260512-contract-parity-monolith | P2 | found | Contract parity validation rules are concentrated in one hardcoded script, making API surface changes fragile to review. | `(Get-Content scripts\validators\contract-parity\engine.py | Measure-Object -Line).Lines` returned `2986`. The same script embeds route-scoped and auth/header rule registries, duplicates tracked schema sets in nested extractors, and hardcodes runtime source paths in `scripts/validators/contract-parity/engine.py`. | Extract shared rule registries and parser helpers before expanding contract parity coverage for additional REST or realtime surfaces. | 2026-05-12T21:30:05Z |
