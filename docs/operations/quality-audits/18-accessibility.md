...
|---|---|---|---|---|---|
| QA-18-20260514-core-inputs-missing-accessible-names | P2 | fixed | Core web text-entry controls rely on placeholders or nearby visual text instead of durable accessible names. | Added `aria-label` and targeted `aria-describedby` coverage to the cited persona, server search, contact invite, invite-use-count, contact search, DM composer, and server-channel composer controls in `apps/web/app/home/page.tsx`, `apps/web/app/servers/page.tsx`, `apps/web/app/contacts/page.tsx`, `apps/web/app/contacts/[contactId]/messages/page.tsx`, and `apps/web/app/servers/[serverId]/page.tsx`; associated the onboarding fingerprint preview label in `apps/web/app/onboarding/identity/page.tsx`; added `apps/web/tests/text-entry-accessible-names.test.ts` as a TSX parser-backed guard for unnamed `input`/`textarea` controls. Temporary harness failed before the fix on the seven cited controls and passed after; `npm --prefix apps/web run test -- tests/text-entry-accessible-names.test.ts` and `npm --prefix apps/web run lint` passed locally. | 2026-05-18T11:06:53Z |
