...
|---|---|---|---|---|---|
| QA-05-20260513-linux-only-ci-tests | P2 | resolved | CI test gates did not exercise the mandatory Windows path. | `.github/workflows/ci.yml` includes `windows-parity-check` on `windows-latest`, running `npm run setup`, profile validators, and `npm run test -- --skip-service-backed-tests`. | 2026-05-20T17:30:00Z |
...
| 2026-05-20T17:30:00Z | Codex | Resolved the Linux-only CI test finding through the Windows parity CI gate and shared Node command surface. |
