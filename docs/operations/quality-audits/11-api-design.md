...
| QA-11-20260513-server-vocabulary-split | P2 | confirmed | Server vocabulary remains split across API surfaces. | Runtime REST and web clients use `/servers` plus `server_id` (`services/api-rs/src/app/router.rs:67`, `apps/web/lib/api.ts:329`), but older target REST and realtime examples previously retained deprecated server aliases in `docs/contracts/mvp-rest.openapi.yaml` and `docs/contracts/realtime-events.asyncapi.yaml`; runtime code still needs follow-up cleanup for legacy compatibility aliases. | Keep `server` as the canonical API vocabulary before new consumers adopt the target-state contracts; remove or explicitly document any remaining compatibility aliases/examples. | 2026-05-13T18:39:23Z |
