...
- Latest meaningful change: 2026-05-14 updated contract-parity guidance after inline and component-referenced REST request-body requiredness checks landed.
...
- `contract-parity-check` also enforces required-field, field-type, nested array item-schema, and referenced item-field parity for a tracked set of high-signal REST DTO schemas: `AuthVerifyRequest`, `AuthVerifyResponse`, `SessionValidateResponse`, `InviteCreateRequest`, `InviteCreateResponse`, `FriendRequestCreateRequest`, `DmFanoutDispatchRequest`, `DmFanoutDispatchResponse`, `DmFanoutCatchUpRequest`, `DmFanoutCatchUpItem`, and `DmFanoutCatchUpResponse`.
