...
- Latest meaningful change: 2026-04-06 broadened contract-parity guidance so auth/social routes now require cleaner route-scoped error examples and clearer social-graph handshake success semantics, server-channel mutation routes document lifecycle success behavior, directory/discovery query surfaces document normalization semantics, and DM fanout/profile-device control responses carry branch-specific success examples and property descriptions.
