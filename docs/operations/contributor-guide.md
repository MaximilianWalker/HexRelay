...
- Latest meaningful change: 2026-05-14 updated contract-parity guidance after tracked REST nested array item-schema checks landed.
...
- `contract-parity-check` also enforces required-field, field-type, and nested array item-schema parity for a tracked set of high-signal REST DTO schemas: `AuthVerifyRequest`, `AuthVerifyResponse`, `SessionValidateResponse`, `InviteCreateRequest`, `InviteCreateResponse`, `FriendRequestCreateRequest`, `DmFanoutDispatchRequest`, `DmFanoutDispatchResponse`, `DmFanoutCatchUpRequest`, `DmFanoutCatchUpItem`, and `DmFanoutCatchUpResponse`.
