...
- Latest meaningful change: 2026-04-06 broadened contract-parity guidance so auth/invite, DM connectivity, and social-graph handshake routes document richer success semantics, auth/social routes keep cleaner route-scoped error examples, server-channel mutation routes document lifecycle success behavior, and directory/discovery query surfaces document normalization semantics.
