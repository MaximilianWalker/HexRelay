...
- `contract-parity-check` also enforces required-field parity for a tracked set of high-signal REST DTO schemas: `AuthVerifyRequest`, `AuthVerifyResponse`, `SessionValidateResponse`, `InviteCreateRequest`, `InviteCreateResponse`, `FriendRequestCreateRequest`, `DmFanoutDispatchRequest`, and `DmFanoutDispatchResponse`.
