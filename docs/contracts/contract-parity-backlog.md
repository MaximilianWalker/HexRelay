...
- Status: validator support is in place and first-pass cleanup landed; auth/invite, DM fanout/profile-device control, server-channel lifecycle, social-graph handshake, DM connectivity setup/control, and the remaining simple block/list/read routes now have branch-specific success descriptions/examples. Remaining work is now close to final residual cleanup rather than missing core capability.
