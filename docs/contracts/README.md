...
- Latest meaningful change: 2026-05-20 aligned runtime contracts with server identity terminology and server-to-server DM forwarding.
...
- Runtime REST server-administration bootstrap uses `GET /server/connection` for public endpoint/auth metadata and authenticated `GET /server/capabilities` for per-identity owner/admin capability reporting.
- Runtime REST DM schemas describe server-to-server P2P E2EE envelope fanout, durable explicit static-peer destination server forwarding, catch-up, and internal ack persistence only; recipient-device pairing, LAN/WAN connectivity, endpoint-card, preflight, and parallel-dial routes are intentionally absent.
- Runtime REST dispatch paths that cross the shared `ServerClientTransport` boundary emit stable snake-case provenance mode/profile/reason-code telemetry without changing REST payload schemas.
