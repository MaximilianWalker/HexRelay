...
- Latest meaningful change: 2026-05-18 aligned contract-parity source watches with transport/app adapter boundaries.
...
- Contract parity watches API runtime handlers, HTTP transport adapters, app-level DM outbound forwarding, realtime app IO services, websocket handlers, shared errors, and selected domain validation/event modules so source moves do not silently drop runtime contract inventory.
