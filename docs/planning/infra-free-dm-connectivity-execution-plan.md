...
- Task IDs: `T4.1.3`
- Outcome: product, architecture, contracts, and CI guardrails enforce ciphertext-only message-server semantics.
- Deliverables:
  ...
  - CI policy checks rejecting server-bypassing DM routes/config/contracts/runtime identifiers.
...
### Phase C: Retire Server-Bypassing DM Surfaces

- Task IDs: `T4.1.5`, `T4.1.6`, `T4.1.11`
- Outcome: server-bypassing DM connectivity surfaces are absent from runtime routes, realtime events, web APIs, contracts, docs, tests, and guardrails.
- Deliverables:
  ...
  - preserve server discovery or voice/media NAT work only where it is explicitly non-DM.

### Phase D: Encrypted-Envelope Message-Server Delivery
...
- Deliverables:
  - message-server ciphertext-envelope accept/store/fanout path.
...
- `T4.1.3`: policy gate report proving unsafe plaintext/key-custody semantics and server-bypassing DM surfaces fail while encrypted-envelope message-server wording passes.
...
- Abuse-control rule: DM dispatch uses sender-scoped rate limits; catch-up and ack use identity/device-scoped rate limits; authenticated server-forward ingress uses origin-server-scoped rate limits.
...
|---|---|---|
| Message-server delivery accidentally expands to server-readable DM content | High | CI guardrail, schema tests, logging tests, and explicit client-only decrypt/key ownership |
...
| Retired server-bypassing DM concepts re-enter through docs/contracts/tests | Medium | Server-bypassing DM guardrail pattern plus contract/runtime negative checks |
...
- Server-bypassing LAN/WAN DM transport, pairing QR/manual-code bootstrap, endpoint hints/cards, preflight, WAN wizard, or parallel dial.
- Broad architecture refactors outside DM delivery and retired server-bypassing DM surface cleanup.
