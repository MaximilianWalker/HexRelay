...
| T1.2.2 | Publish runtime REST OpenAPI contract artifact | E1 / S1.2 | S | API | T1.1.1 | `docs/contracts/runtime-rest.openapi.yaml` is committed and referenced by T2 tasks |
...
| T2.1.1 | Implement key identity schema + key registration endpoints | E2 / S2.1 | XL | API | T1.1.1, T1.1.2, T1.2.2 | Identity keys can be registered and validated with tests; schema follows MVP Crypto Profile in `docs/product/01-mvp-plan.md` |
...
| T1.2.2 | `docs/contracts/runtime-rest.openapi.yaml` | OpenAPI file exists, includes all six runtime identity/auth/invite endpoints, and is linked from board/plan |
...
- `docs/contracts/runtime-rest.openapi.yaml` exists before starting `T2.*` tasks.
