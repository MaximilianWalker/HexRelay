# Planning Docs

## Document Metadata

- Doc ID: planning-index
- Owner: Delivery maintainers
- Status: ready
- Scope: repository
- last_updated: 2026-05-04
- Source of truth: `docs/planning/README.md`

## Quick Context

- Primary edit location for this document's canonical topic.
- Update this file when its source-of-truth topic changes.
- Latest meaningful change: 2026-05-04 added the canonical local runtime testing plan for fixture profiles, multi-instance launch, and network simulation.

- `Status: ready` means this index is the canonical planning router, not that every deferred runtime or release-readiness gap is closed; check the `Active Watch Summary` in `docs/operations/readiness-corrections-log.md` before planning against unsettled behavior.

## Contents

- Iteration boards index (canonical execution planning source): `docs/planning/iterations/README.md`
- Infrastructure-free DM connectivity execution plan: `docs/planning/infra-free-dm-connectivity-execution-plan.md`
- Cross-scenario networking architecture authority: `docs/architecture/04-communication-networking-layer-plan.md`
- Iteration change log: `docs/planning/05-iteration-log.md`
- KPI/SLO fixed benchmark profile: `docs/planning/kpi-slo-test-profile.md`
- TURN/NAT constrained-network profile: `docs/planning/turn-nat-test-profile.md`
- Local runtime testing plan: `docs/planning/local-runtime-testing-plan.md`
- MVP verification matrix: `docs/testing/01-mvp-verification-matrix.md`
- Use `docs/planning/sprint-board-template.md` for new iterations.
