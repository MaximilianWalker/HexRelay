...
- Latest meaningful change: 2026-03-23 T4.3.4 added websocket verification for both `channel.message.created` and `channel.message.updated` active-device fanout, late-device hydration, and no-duplicate replay on second reconnect using the shared Redis-backed profile-device replay model.
