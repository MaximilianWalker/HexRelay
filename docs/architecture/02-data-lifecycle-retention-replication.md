...
- Latest meaningful change: 2026-05-11 aligned DM envelope ownership with server-node P2P message-node storage and durable outbound retry scheduling while keeping plaintext/private keys client-only.
...
| DM outbound server-node forwarding attempts | origin server node | optional cache | yes (ciphertext envelope plus minimal routing/transport state only) | retry/failure bookkeeping, attempt count, and next-attempt schedule are origin-node state; recipient-device delivery remains ack-backed and destination-owned |
