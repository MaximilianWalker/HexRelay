...
3. **Terminal B**: verify API and realtime health using the URLs printed by `npm run run`.
4. **Terminal C**: open the printed web URL and continue with local testing.
5. **Terminal D**: run `npm run test`.

Expected result: `npm run run` prints a ready stack summary with API/realtime/web URLs, health checks return success on those printed URLs, and tests pass before starting feature implementation.
